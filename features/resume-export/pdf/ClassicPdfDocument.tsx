import { Document, Page, StyleSheet, Text, View } from "@react-pdf/renderer";

import { buildContactLine } from "@/features/resume-preview/buildContactLine";
import { formatDateRange } from "@/features/resume-preview/dateFormatting";
import type { PreviewResumeData } from "@/features/resume-preview/types";

const styles = StyleSheet.create({
  page: { padding: 32, fontSize: 10, fontFamily: "Helvetica", color: "#000000" },
  header: {
    textAlign: "center",
    marginBottom: 12,
    borderBottomWidth: 1,
    borderBottomColor: "#000000",
    paddingBottom: 8,
  },
  name: { fontSize: 18, fontFamily: "Helvetica-Bold", letterSpacing: 1 },
  contactLine: { fontSize: 9, marginTop: 4 },
  section: { marginBottom: 10 },
  sectionTitle: {
    fontSize: 10,
    fontFamily: "Helvetica-Bold",
    borderBottomWidth: 1,
    borderBottomColor: "#000000",
    marginBottom: 4,
    textTransform: "uppercase",
  },
  entry: { marginBottom: 6 },
  entryHeader: { flexDirection: "row", justifyContent: "space-between" },
  entryHeaderText: { fontFamily: "Helvetica-Bold" },
  bullet: { flexDirection: "row", marginLeft: 8, marginTop: 2 },
  bulletDot: { width: 8 },
  bulletText: { flex: 1 },
});

interface Props {
  data: PreviewResumeData;
}

/** react-pdf equivalent of features/resume-preview/components/ClassicTemplate.tsx — same content and structure, react-pdf's own primitives instead of HTML/Tailwind. */
export function ClassicPdfDocument({ data }: Props) {
  const contactLine = buildContactLine(data.contact);

  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.name}>{data.contact.fullName.toUpperCase()}</Text>
          {contactLine ? <Text style={styles.contactLine}>{contactLine}</Text> : null}
        </View>

        {data.summary ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Summary</Text>
            <Text>{data.summary}</Text>
          </View>
        ) : null}

        {data.skills.length > 0 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Skills</Text>
            <Text>{data.skills.map((skill) => skill.name).join(", ")}</Text>
          </View>
        ) : null}

        {data.experience.length > 0 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Experience</Text>
            {data.experience.map((entry) => (
              <View key={entry.id} style={styles.entry}>
                <View style={styles.entryHeader}>
                  <Text style={styles.entryHeaderText}>
                    {entry.role} — {entry.company}
                  </Text>
                  <Text>{formatDateRange(entry.startDate, entry.endDate)}</Text>
                </View>
                {entry.bullets.map((bullet, index) => (
                  <View key={index} style={styles.bullet}>
                    <Text style={styles.bulletDot}>•</Text>
                    <Text style={styles.bulletText}>{bullet}</Text>
                  </View>
                ))}
              </View>
            ))}
          </View>
        ) : null}

        {data.projects.length > 0 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Projects</Text>
            {data.projects.map((entry) => (
              <View key={entry.id} style={styles.entry}>
                <Text style={styles.entryHeaderText}>{entry.name}</Text>
                {entry.description ? <Text>{entry.description}</Text> : null}
                {entry.bullets.map((bullet, index) => (
                  <View key={index} style={styles.bullet}>
                    <Text style={styles.bulletDot}>•</Text>
                    <Text style={styles.bulletText}>{bullet}</Text>
                  </View>
                ))}
              </View>
            ))}
          </View>
        ) : null}

        {data.education.length > 0 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Education</Text>
            {data.education.map((entry, index) => (
              <View key={index} style={styles.entryHeader}>
                <Text>
                  {entry.degree}
                  {entry.institution ? `, ${entry.institution}` : ""}
                </Text>
                <Text>{formatDateRange(entry.startDate ?? "", entry.endDate)}</Text>
              </View>
            ))}
          </View>
        ) : null}

        {data.certifications.length > 0 ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Certifications</Text>
            {data.certifications.map((entry, index) => (
              <Text key={index}>
                {entry.name}
                {entry.issuer ? ` — ${entry.issuer}` : ""}
              </Text>
            ))}
          </View>
        ) : null}
      </Page>
    </Document>
  );
}
